...
import { getAllSubSegmentsWithId } from "src/lib/api/segment.routes";
import type { ISegment, ISubSegment } from "@/types/segment.types";
...
const NeighborhoodDropdown: React.FC<NeighborhoodDropdownProps> = ({
  ...
  return (
    <Form.Group controlId="neighborhood">
      ...
      >
        <option value={DEFAULT_NEIGHBORHOOD.value}>
          {DEFAULT_NEIGHBORHOOD.label}
        </option>
        {subSegments &&
          [...subSegments]
            .sort((a, b) => {
            ...
            .map((subSegment: any) => {
              return (
                <option key={subSegment.id} value={subSegment.name}>
...
export const SegmentInfo: React.FC<SegmentInfoProps> = ({
  ...
  function handleCommunityChange(
    segName: FormDataEntryValue,
    subSegName: FormDataEntryValue,
  ) {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setFirstNameMessage(
      data.displayFName ? "" : "Please provide a first name.",
    );
    ...
    setCityMessage(
      data.city && data.city !== "Not Selected"
        ? ""
        : "Please select a municipality.",
    );
    setPostalCodeMessage(
      data.postalCode ? "" : "Please provide a postal code / zip.",
    );
    ...
    <>
        ...
        <Row>
          ...
          {edit ? (
              ...
              <Col>
                <Form.Group controlId="displayName">
                  ...
                  <Form.Control
                    ...
                    defaultValue={capitalize(displayFName)}
                  ...
                  <Form.Control
                    ...
                    defaultValue={capitalize(displayLName)}
                ...
                <Form.Group controlId="street">
                  ...
                  <Form.Control
                    ...
                    type="text"
                    placeholder="Street"
                    defaultValue={capitalize(street)}
                    isInvalid={!!streetMessage}
                  ></Form.Control>
                  <Form.Control.Feedback type="invalid">
                    {streetMessage}
                  </Form.Control.Feedback>
                ...
                <Form.Group controlId="city">
                  ...
                  <Form.Control
                    ...
                    value={capitalize(formCity)}
                    readOnly
                    plaintext
                    isInvalid={!!cityMessage}
                  ></Form.Control>
                  <Form.Control.Feedback type="invalid">
                    {cityMessage}
                  </Form.Control.Feedback>
                ...
                <Form.Group controlId="postalCode">
                  ...
                  <Form.Control
                    ...
                    defaultValue={capitalize(postalCode)}
                    isInvalid={!!postalCodeMessage}
                  ></Form.Control>
                  <Form.Control.Feedback type="invalid">
                    {postalCodeMessage}
                  </Form.Control.Feedback>
            ...
            <>
              ...
              <ListGroup variant="flush">
                <ListGroupItem>
                  {capitalize(displayFName)}@{capitalize(displayLName)}
                </ListGroupItem>
                {/* <ListGroupItem>{capitalizeString(user.userSegments?.homeSegHandle)}</ListGroupItem> */}
                <ListGroupItem>{capitalize(street)}</ListGroupItem>
                <ListGroupItem>{capitalize(formCity)}</ListGroupItem>
                <ListGroupItem>{postalCode.toUpperCase()}</ListGroupItem>
                <ListGroupItem>{capitalize(formNeighborhood)}</ListGroupItem>
              </ListGroup>
              <Col style={{ maxWidth: "10rem" }}>
                <Button
                  variant="primary"
                  className=""
                  onClick={handleUpdateNeighborhoodOnly}
                >
                  Edit
                </Button>
        ...
        <Modal.Body>
          ...
          >
            <Form.Group controlId={type + "Segment"}>
              ...
              >
                <option value={DEFAULT_MUNICIPALITY.value}>
                  {DEFAULT_MUNICIPALITY.label}
                </option>
                {segments &&
                  segments
                    .filter(
                      (segment: ISegment) =>
                        segment.name.toLowerCase() !== "notselected",
                    )
      ...
      <Modal show={showDelete} onHide={handleDeleteClose}>
        ...
        <Modal.Body>
          <p>
            <strong>
              You are about to delete all of your information on your {type}{" "}
              community.
            </strong>
          </p>
