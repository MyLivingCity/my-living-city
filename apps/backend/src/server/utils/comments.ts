...
export async function funnelCommentApi(newComment) {
  ...
  let commentKeywords = [];
  if (completion) {
    commentKeywords = completion.choices[0].message.content
      .toLowerCase()
      .split(" | ");
  }
...
export async function checkSimilar(commentKeywords, ideaId, loggedInUser) {
  const enabled = await isCheckSimilarEnabled();

  const similarCommentArray = [];
  const similarityCountArray = [];
  ...
  if (enabled) {
    ...
    while (similarCommentArray.length > 5) {
      ...
      const minIndex = similarityCountArray.indexOf(
        Math.min(...similarityCountArray),
      );
