...
const ProposalTile: React.FC<proposalTileProps> = ({
  ...
  const superSegmentName =
    superSegments.find(
      (superSegment) =>
        superSegment.superSegId === proposalData.idea.superSegId,
    )?.name || "";
  ...
  return (
    ...
    <Card>
      ...
      <Card.Body>
        <div className="mb-1" style={{ textAlign: "left", color: "gray" }}>
          {postType}
        </div>
        <Card.Title>{idea ? truncateString(idea.title, 50) : "N/A"}</Card.Title>
        <small className="text-muted">
          By: {proposalData.idea.firstName || "Unknown"}
        </small>
            ...
            <small className="text-muted user-select-none">
              {idea.subSegmentName
                ...
                : idea.segmentName
                  ...
                  : superSegmentName
                    ? ` ${capitalizeString(superSegmentName)}`
                    : ""}
            </small>
          </div>
        </Card.Footer>
